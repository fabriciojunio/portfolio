...
function IconBtn({ label, active, onClick, children }: IconBtnProps) {
  return (
    <button
      ...
      aria-label={label}
      onClick={onClick}
      className={`relative w-10 h-10 flex items-center justify-center text-[#6b6b6b] hover:text-[#ededed] transition-colors ${active ? "text-[#ededed]" : ""}`}
    >
      {active && (
        <span className="absolute left-0 top-1 bottom-1 w-[2px] bg-[#ffffff]" />
      )}
      {children}
    </button>
  );
...
export default function Activity({ onExplorer, onSearch, onRun }: ActivityProps) {
  return (
    <nav
      ...
      className="w-10 bg-[#0c0c0c] border-r border-[#1c1c1c] flex flex-col items-center py-1 select-none"
    >
      ...
      <a
        ...
        title="GitHub"
        aria-label="GitHub"
        className="w-10 h-10 flex items-center justify-center text-[#6b6b6b] hover:text-[#ededed]"
      >
